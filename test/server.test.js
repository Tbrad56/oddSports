...
test('proxies a valid sport, appends key upstream, passes body and quota header back', async () => {
  ...
  assert.match(f.calls[0], /^https:\/\/api\.the-odds-api\.com\/v4\/sports\/basketball_nba\/odds\/\?regions=us&markets=h2h&oddsFormat=american&apiKey=sekret$/);
});
...
test('props: proxies valid request with server-side market list', async () => {
  ...
  assert.match(f.calls[0], /\/v4\/sports\/basketball_nba\/events\/0a1b2c3d4e5f\/odds\/\?regions=us&markets=player_points,player_rebounds,player_assists,player_threes,player_points_rebounds_assists&oddsFormat=american&apiKey=k$/);
});
